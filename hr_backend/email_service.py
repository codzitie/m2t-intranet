...
def send_password_reset_otp(email: str, otp_code: str, username: str) -> bool:
    """Send password reset OTP via email with beautiful HTML template"""
...
<body style="margin:0;padding:0;font-family:Arial,sans-serif;background:#f5f7fa;">
            ...
            <td align="center" style="padding:40px 20px;">
                <table width="520px" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:12px;box-shadow:0 4px 24px rgba(37,99,235,0.08);">
                    <tr>
                        <td style="background:linear-gradient(135deg,#dc2626,#ef4444);padding:32px;text-align:center;border-radius:12px 12px 0 0;">
                            <div style="font-size:42px;margin-bottom:8px;">🔐</div>
                            <h1 style="margin:0;color:#fff;font-size:20px;font-weight:700;">Password Reset Request</h1>
                            <p style="margin:6px 0 0;color:rgba(255,255,255,0.85);font-size:13px;">M2T HR Portal Security</p>
                        </td>
                    </tr>
                    <tr>
                        <td style="padding:32px 28px;">
                            <h2 style="margin:0 0 6px;color:#1e293b;font-size:18px;">Hello {username},</h2>
                            <p style="margin:0 0 24px;color:#64748b;font-size:14px;">We received a request to reset your password. Use this code to proceed:</p>
                            <div style="background:linear-gradient(135deg,#fff5f5,#fef2f2);border:1.5px solid #fecaca;border-radius:10px;padding:20px;text-align:center;">
                                <p style="margin:0 0 8px;color:#64748b;font-size:11px;font-weight:700;text-transform:uppercase;">Password Reset Code</p>
                                <div style="font-size:32px;font-weight:900;color:#dc2626;letter-spacing:8px;font-family:'Courier New',monospace;margin:8px 0;">{otp_code}</div>
                                <p style="margin:12px 0 0;background:rgba(220,38,38,0.08);color:#991b1b;font-size:12px;font-weight:600;padding:6px 14px;border-radius:6px;display:inline-block;">⏱ Expires in 10 minutes</p>
                            </div>
                            <div style="background:#fff7ed;border-left:3px solid#f59e0b;padding:14px 16px;border-radius:6px;margin-top:24px;">
                                <p style="margin:0;color:#92400e;font-size:13px;"><strong>⚠️ Security Notice:</strong> If you didn't request this password reset, please ignore this email and your password will remain unchanged.</p>
                            </div>
                        </td>
                    </tr>
                    <tr>
    ...
    if not all([settings.SMTP_HOST, settings.SMTP_USER, settings.SMTP_PASS]):
        print(f"\n{'='*50}")
        print(f"🔐 PASSWORD RESET OTP (Console Fallback)")
        print(f"{'='*50}")
        print(f"To: {email}")
        print(f"User: {username}")
        print(f"\n🔑 RESET CODE: {otp_code}\n")
